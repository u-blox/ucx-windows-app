...
class UCXHeaderParser:
    ...
    def parse_parameters(self, params_str: str) -> List[FunctionParam]:
        ...
        for part in param_parts:
            ...
            # Handle complex types like "const char *", "uint8_t *", "int32_t"
            tokens = part.split()
            ...
            # Check for pointers
            pointer_count = part.count('*')
            full_type = ' '.join(type_tokens)
            if pointer_count > 0:
                full_type += ' ' + '*' * pointer_count
            
            # Determine base C type
            c_type = ' '.join(type_tokens).replace('const', '').strip()
...
class CWrapperGenerator:
    ...
    def generate_function_declaration(self, func: Function) -> str:
        ...
        wrapper_name = f"ucx_{func.module}_{func.name.replace('uCx' + func.module.title().replace('_', ''), '').lstrip('_')}"
        if not wrapper_name.startswith('ucx_'):
            wrapper_name = f"ucx_{func.name.replace('uCx', '').lower()}"
...
class CSharpPInvokeGenerator:
    ...
    def generate(self) -> str:
        ...
        for module in modules:
            ...
            for func in funcs[:10]:  # Limit for now
    ...
    def generate_pinvoke_declaration(self, func: Function) -> str:
        ...
        for param in func.params:
            ...
            cs_type = self.map_csharp_type(param.c_type, is_pointer=('*' in param.full_type))
