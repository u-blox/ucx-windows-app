...
class CWrapperGenerator:
    ...
    def generate_header(self) -> str:
        """Generate C header file with proper UCX API includes"""
        code = []
        ...
        # Include UCX API headers - these provide all type definitions
        ...
        # Forward declaration for ucx_instance
        code.append("/* Forward declaration */")
        code.append("typedef struct ucx_instance ucx_instance_t;\n")
        
        # Generate function declarations by module
        ...
        for module in modules:
            ...
            for func in funcs:
                code.append(self.generate_function_declaration(func))
    ...
    def generate_implementation(self) -> str:
        ...
        # Include headers - put all UCX includes FIRST before our header
        code.append("/* Include all UCX API headers first (before wrapper header) */")
        code.append('#include "../ucxclient/ucx_api/u_cx.h"')
        ...
        code.append("")
        code.append("/* Now include wrapper headers */")
        code.append('#include "ucxclient_wrapper_internal.h"')
...
class CSharpPInvokeGenerator:
    ...
    def map_csharp_type(self, c_type: str, is_return: bool = False, is_pointer: bool = False) -> str:
        ...
        if base_type in type_map:
            ...
            # Unknown types (enums, structs) - use IntPtr for structs, int for enums
            # Heuristic: if it ends with _t and contains uppercase, likely a struct -> IntPtr
            # Otherwise (enum types like uBtMode_t) -> int
            if '_t' in base_type and not is_pointer:
                # For value parameters of unknown types, use int (enums are ints in C)
                cs_type = 'int'
            else:
                cs_type = base_type
